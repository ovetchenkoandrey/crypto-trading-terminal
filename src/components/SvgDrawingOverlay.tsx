...
function DrawingShape({ ctx, drawing, selected, onClick, allowClick }: ShapeProps) {
  ...
  const clickProps = allowClick
    ? { onClick: (e: React.MouseEvent) => { e.stopPropagation(); onClick(); }, style: { cursor: "pointer", pointerEvents: "stroke" as const } }
  ...
  if (drawing.kind === "trendline") {
    ...
    return (
      <g>
        <line x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke={stroke} strokeWidth={sw} strokeDasharray={dash} {...clickProps} />
        {allowClick && (
          <line x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="transparent" strokeWidth={HIT_DISTANCE * 2}
                onClick={(e) => { e.stopPropagation(); onClick(); }} style={{ cursor: "pointer" }} />
        )}
        {selected && (
          <>
    ...
    return (
      <g {...clickProps}>
        {drawing.levels.map((lvl) => {
          const y = (a.y as number) + ((b.y as number) - (a.y as number)) * lvl;
        ...
        {/* the trendline that spans the fib */}
        <line x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke={stroke} strokeWidth={sw} opacity={0.6} />
        {selected && (
          <>
            <circle cx={a.x} cy={a.y} r={4} fill={stroke} />
            <circle cx={b.x} cy={b.y} r={4} fill={stroke} />
  ...
  if (drawing.kind === "text") {
    ...
    return (
      <g {...clickProps}>
        <text x={p.x} y={p.y} fill={stroke} fontSize={drawing.fontSize ?? 12} fontFamily='"Segoe UI", sans-serif'
